...
- Single-entrypoint CLI for every ElevenLabs audio surface beyond Ysmara's own TTS. Replaces the sprawl of one-off inline scripts with three subcommands:
  - `sfx <prompt|preset> [--seconds N] [--keep] [--out PATH]` — prompt-to-audio via `/v1/sound-generation`. Preset aliases expand to curated prompts (17 baked in: `chime`, `ping`, `thud`, `whoosh`, `coin`, `rimshot`, `airhorn`, `cheer`, `fanfare`, `scratch`, `kazoo`, `sad_trombone`, `ship_it`, `oops`, `wake`, `sleep`, `typing`).
  - `preset ls` — prints the curated preset table.
  - `voice ls [--search Q]` / `voice mine` / `voice try <voice-id> <text>` — browse the EL shared-voice library with search filtering, list your own custom voices, or do a one-off TTS render in any voice without touching `voice/speak.py`'s Ysmara config.
- Reuses the Windows MCI playback path proven in `speak.py`. No PowerShell/COM risk, no ffmpeg dependency.
- Orthogonal to `voice/speak.py` — Ysmara's persona + lip-sync path is unchanged. `audio.py` is for everything else.

### Added — Conversational speech-state HUD (listening / heard / thinking / speaking)
...
- UTF-8 stdout hardening — parent stdout reconfigures to `encoding="utf-8", errors="replace"` at module import; the pipe thread falls back to ASCII-safe replacement on `UnicodeEncodeError` so a single smart-quote from a child can't silently kill the pipe and stall the whole console.
